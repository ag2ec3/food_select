...
| GET | `/api/teams/[teamId]` | 단일 팀 조회 (`id`, `name`, `created_at`). 비멤버·없는 팀은 404 |
| GET | `/api/teams/[teamId]/members` | 같은 팀 멤버 목록 (`user_id`, `role`, `joined_at`, 가입일 오름차순). 비멤버는 403 |
| POST | `/api/teams/join` | 팀 참가. Body `{ "invite_code": "..." }`. 성공 시 `{ "ok": true, "team_id": "..." }` |
| POST | `/api/teams/[teamId]/join` | (선택) URL에 팀 ID를 아는 경우 동일 초대 코드로 참가 |
...
- 응답: `{ "members": [ { "user_id": "uuid", "role": "owner" | "member", "joined_at": "..." }, ... ] }`
- 팀에 속하지 않은 사용자: `403` + `{ "error": "..." }`
...
| GET | `/api/teams/[teamId]/sessions/[sessionId]` | 세션 상세 조회 (후보·투표·확정 결과 포함). 응답에 `team_name` 포함 |
...
4. Decision 레코드 생성과 세션 `closed` 갱신은 DB 함수 `close_session_pick_winner` 한 트랜잭션에서 수행한다 (중간 실패로 상태가 어긋나지 않도록).
